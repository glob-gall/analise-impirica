...
with open('metrics.csv', 'w', newline='') as csvfile:
    ...
    writer.writerow(['Arquivo', 'LOC', 'LLOC', 'SLOC', 'Linhas de Comentário', 'Linhas em Branco', 'Complexidade Ciclomática'])
    ...
    for filename in file_list:
        try:
            ...
            writer.writerow([filename, loc, lloc, sloc, comment_lines, blank_lines, total_complexity])

        except Exception as e:
            print(f"Erro ao processar o arquivo {filename}: {str(e)}")
            writer.writerow([filename, 0, 0, 0, 0, 0, 0])
